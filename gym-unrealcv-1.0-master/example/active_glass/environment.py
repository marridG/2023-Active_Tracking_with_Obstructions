...
def create_env(env_id, args, mp_name):
    # if '2D' in env_id:
    #     import gym_track2d
    # if 'Unreal' in env_id:
    # print(args.intrinsic_reward)
    import gym_unrealcv
    env = gym.make(env_id)
    env.env.env.env.env.glass_detect_dir = mp_name  # dirname to save mask detection results
    # config observation pre-processing
    if args.single: #   True
        env = listspace(env)
    if args.rescale is True:    # False
        env = Rescale(env, args)  # rescale, inv
    if 'img' in args.obs and '2D' not in env_id:    # True
...
class UnrealPreprocess(gym.ObservationWrapper):
    def __init__(self, env, args):
        ...
        self.gray = args.gray   # False
        self.crop = args.crop   # True
        self.input_size = args.input_size   # 80

        obs_shape = self.observation_space[0].shape
        num_agnets = 1# len(self.observation_space)
    ...
    def observation(self, observation):
        ...
        return np.array(obses)  # 1 * 1 * 80 * 80
